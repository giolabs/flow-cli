...
def mock_flutter_project_with_git(mock_flutter_project, temp_dir):
    ...
    subprocess.run(['git', 'config', 'user.email', 'test@example.com'], cwd=mock_flutter_project, capture_output=True)
    subprocess.run(['git', 'config', 'user.name', 'Test User'], cwd=mock_flutter_project, capture_output=True)
    subprocess.run(['git', 'add', '.'], cwd=mock_flutter_project, capture_output=True)
    subprocess.run(['git', 'commit', '-m', 'Initial commit'], cwd=mock_flutter_project, capture_output=True)
...
def sample_config():
    ...
    return {
        'flutter': {
            'sdk_path': '/usr/local/flutter',
            'channel': 'stable'
        },
        'android': {
            'sdk_path': '/Users/test/Library/Android/sdk',
...
def assert_command_success(result, expected_output=None):
    ...
    if expected_output:
        assert expected_output in result.output, f"Expected output '{expected_output}' not found in: {result.output}"
...
def assert_command_failure(result, expected_exit_code=1, expected_error=None):
    ...
    if expected_error:
        assert expected_error in result.output, f"Expected error '{expected_error}' not found in: {result.output}"
...
def assert_performance_under_threshold(elapsed_time, threshold_seconds):
    ...
    assert elapsed_time < threshold_seconds, f"Command took {elapsed_time}s, expected under {threshold_seconds}s"