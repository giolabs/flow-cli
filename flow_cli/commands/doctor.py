...
def check_flutter(verbose: bool) -> Tuple[str, str, str, str]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            doctor_result = subprocess.run(['flutter', 'doctor', '--no-version-check'], 
            ...
            return ("Flutter SDK", "❌", "Not found", "Install Flutter SDK from https://flutter.dev")
    except (subprocess.TimeoutExpired, FileNotFoundError):
        return ("Flutter SDK", "❌", "Not found or not responding", "Install Flutter SDK from https://flutter.dev")
        ...
        if android_home.exists():
            ...
            if adb_path.exists():
                ...
                if result.returncode == 0:
                    ...
                    return ("Android SDK", "⚠️", "SDK found, ADB issues", "ADB may not be working properly")
            else:
...
def check_ios(verbose: bool) -> Tuple[str, str, str, str]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            sim_result = subprocess.run(['xcrun', 'simctl', 'list', 'devices'], 
                                      capture_output=True, text=True, timeout=10)
        ...
        return ("iOS Development", "❌", "Error checking Xcode", "Unable to verify Xcode installation")
...
def check_python_packages(verbose: bool) -> Tuple[str, str, str, str]:
    ...
    required_packages = [
        'click', 'rich', 'inquirer', 'pyyaml', 'requests', 'colorama', 'Pillow', 'ruamel.yaml'
    ]
    ...
    if not missing_packages:
        return ("Python Packages", "✅", "All packages available", f"All {len(required_packages)} packages found")
    else:
        missing_str = ", ".join(missing_packages)
        return ("Python Packages", "⚠️", f"{len(missing_packages)} missing", f"Missing: {missing_str}")
