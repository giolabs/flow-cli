...
def setup_fastlane_command(force: bool, skip_branch: bool) -> None:
    ...
    if fastfile_path.exists() and not force:
        ...
        console.print("Use --force to reconfigure or run 'flow deployment status' to check configuration")
        return
...
• keys/ directory (for certificates)"""
    ...
    panel = Panel(
        warning_text,
        title="🔒 Branch Safety Protection",
        border_style="red",
        box=box.ROUNDED
    )
    console.print(panel)
...
def confirm_proceed() -> bool:
    ...
    try:
        return inquirer.confirm(
            "Do you want to proceed with Fastlane setup?",
            default=True
        )
...
def create_feature_branch(project: FlutterProject) -> bool:
    ...
    try:
        ...
        if uncommitted:
            ...
            subprocess.run(['git', 'stash', 'push', '-m', 'Flow CLI: Pre-Fastlane setup stash'], 
...
def interactive_fastlane_config(project: FlutterProject) -> Optional[dict]:
    ...
    try:
        questions = [
            ...
            inquirer.Text(
                'developer_name',
                message="Developer/Company name",
...
def configure_ios_settings() -> dict:
    ...
    try:
        questions = [
            ...
            inquirer.Text(
                'itc_team_id',
                message="App Store Connect Team ID (optional)",
                default=""
            ),
            inquirer.Confirm(
                'upload_to_app_store',
