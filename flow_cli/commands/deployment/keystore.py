...
@click.command()
@click.option('--platform', type=click.Choice(['android', 'ios', 'both']), default='both', help='Platform to generate keystore for')
...
def generate_android_keystore(project: FlutterProject, keys_dir: Path, config: Dict[str, Any], force: bool) -> Dict[str, Any]:
    ...
    with Progress(
        ...
        try:
            ...
            keytool_cmd = [
                ...
                '-validity', '10000',
                '-alias', 'release',
                '-storepass', config['android_keystore_password'],
                '-keypass', config['android_key_password'],
                '-dname', f"CN={config['company_name']}, OU={config['app_name']}, O={config['company_name']}, L={config['city']}, S={config['state']}, C={config['country_code']}"
            ]
...
def generate_ios_certificates(project: FlutterProject, keys_dir: Path, config: Dict[str, Any], force: bool) -> Dict[str, Any]:
    ...
    with Progress(
        ...
        try:
            ...
            task = progress.add_task("Generating iOS certificates with Fastlane Match...", total=None)
