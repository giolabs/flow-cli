...
def analyze_ios_flavor(project: FlutterProject, flavor: str) -> Dict:
    ...
    scheme_path = (
        ios_dir / "Runner.xcodeproj" / "xcshareddata" / "xcschemes" / f"{scheme_name}.xcscheme"
    )
    ...
    ios_configured = (
        has_scheme
        and has_info_plist
        and plist_data.get("CFBundleIdentifier")
        and plist_data.get("CFBundleDisplayName")
    )
        ...
        if schemes:
            ...
            for scheme in schemes:
                ...
                status = (
                    ...
                    else "[yellow]⚠️ Shared needed[/yellow]"
                )
...
def show_ios_project_info(project: FlutterProject) -> None:
    ...
    if project_info:
        info_text = "\n".join(project_info)
        info_panel = Panel(
            info_text, title="🍎 iOS Project Status", border_style="blue", box=box.ROUNDED
        )
...
• open ios/Runner.xcodeproj     # Open in Xcode"""
    ...
    panel = Panel(message, title="💡 Setting Up iOS Flavors", border_style="blue", box=box.ROUNDED)
    console.print(panel)
