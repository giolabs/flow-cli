...
def analyze_project_structure(project: FlutterProject, flavor: Optional[str], verbose: bool) -> Dict:
...
def display_text_results(results: Dict, project: FlutterProject, verbose: bool) -> None:
    ...
    if 'analyze_flutter_code' in results:
        ...
        if 'error' not in code_result:
            status = "✅ Clean" if code_result['no_issues'] else f"⚠️ {code_result['issues_count']} issues"
