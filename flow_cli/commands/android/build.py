...
@click.option('--mode', '-m', type=click.Choice(['debug', 'release', 'profile']), default='debug', help='Build mode')
@click.option('--format', type=click.Choice(['apk', 'appbundle']), default='apk', help='Output format')
...
def build_command(flavor: Optional[str], mode: str, format: str, all_flavors: bool, verbose: bool) -> None:
    """
    ...
    if all_flavors:
        ...
        if flavor:
            if flavor not in project.flavors:
                show_error(f"Flavor '{flavor}' not found. Available flavors: {', '.join(project.flavors)}")
                raise click.Abort()
...
def build_single_flavor(project: FlutterProject, flavor: str, mode: str, format: str, verbose: bool) -> dict:
    ...
    with Progress(
        ...
        try:
            ...
            return {
                ...
                'size_mb': get_file_size_mb(output_path) if output_path and output_path.exists() else 0
            }
...
def get_build_output_path(project: FlutterProject, flavor: str, mode: str, format: str) -> Optional[Path]:
...
def display_build_results(build_results: List[tuple], mode: str, format: str) -> None:
    ...
    for flavor_name, result in build_results:
        if result['success']:
            ...
            output_path = str(result['output_path'].relative_to(Path.cwd())) if result['output_path'] else "Not found"
        else:
...
def show_build_failures(failed_builds: List[tuple]) -> None:
    ...
    for flavor_name, result in failed_builds:
        ...
        error_panel = Panel(
            error_text,
            title=f"❌ Build Error: {flavor_name}",
            border_style="red",
            box=box.ROUNDED
        )
