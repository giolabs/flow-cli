...
def interactive_flavor_selection(flavors: List[str]) -> tuple:
    ...
    try:
        questions = [
            inquirer.List(
                'selection',
                message="Select target for icon generation:",
                choices=choices
            )
        ]
...
def generate_all_flavor_icons(project: FlutterProject, flavors: List[str], platform: str) -> None:
    ...
    with Progress(
        ...
        for flavor in flavors:
            ...
            if not icon_path.exists():
                results.append((flavor, {'success': False, 'error': f'Icon file not found: {icon_path}'}))
...
def run_flutter_launcher_icons(project: FlutterProject, config: Dict, flavor: Optional[str] = None) -> Dict:
    """Run flutter_launcher_icons with given configuration"""
    
    # Create temporary config file
    config_filename = f"flutter_launcher_icons_{flavor}.yaml" if flavor else "flutter_launcher_icons.yaml"
