...
@click.option('--platform', type=click.Choice(['android', 'ios', 'both']), default='both', help='Target platform')
...
def interactive_flavor_selection(flavors: List[str]) -> tuple:
    ...
    try:
        questions = [
            inquirer.List(
                'selection',
                message="Select target for splash screen generation:",
                choices=choices
            )
        ]
...
def generate_all_flavor_splash(project: FlutterProject, flavors: List[str], platform: str) -> None:
    ...
    with Progress(
        ...
        for flavor in flavors:
            ...
            if not splash_path.exists():
                results.append((flavor, {'success': False, 'error': f'Splash image not found: {splash_path}'}))
...
def create_splash_config(flavor: Optional[str], platform: str, splash_path: str, flavor_config: Dict = {}) -> Dict:
...
def run_flutter_native_splash(project: FlutterProject, config: Dict, flavor: Optional[str] = None) -> Dict:
    """Run flutter_native_splash with given configuration"""
    
    # Create temporary config file
    config_filename = f"flutter_native_splash_{flavor}.yaml" if flavor else "flutter_native_splash.yaml"
